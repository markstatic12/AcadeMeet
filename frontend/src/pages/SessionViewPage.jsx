...
const SessionViewPage = () => {
  ...
  const requiresPasswordAuth = useCallback((sessionData) => {
    ...
    return sessionData.sessionPrivacy === 'PRIVATE' && !isOwner;
  }, [currentUserId]);
  ...
  const handleJoinSession = async () => {
    if (isParticipant) {
      ...
      const passwordToUse = session.sessionPrivacy === 'PRIVATE' ? validatedPassword : null;
              ...
              {(session.status === 'ACTIVE' || session.status === 'SCHEDULED') && (
                ...
                >
                  {isSessionOwner ? (
                    ...
                    <>
                      {session.sessionPrivacy === 'PRIVATE' && !isParticipant && (
